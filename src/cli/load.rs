...
use crate::{bootstrap, connect, pragma, process_dir, Error, Options};

#[derive(Debug, Clap)]
pub struct Cmd {
    /// Path to the SQLite database.
    #[clap(long, value_name = "db_path", default_value = "./addressbase.sqlite")]
    db_path: String,
...
impl Cmd {
    pub fn run(&mut self) -> Result<&str, Error> {
        ...
        let msg = process_dir(&mut conn, &data_path, fpt)?;
